...
%%% RESUME WORKING HERE %%%		L 	O 	O 	K 		H 	E 	R 	E
...
The following paragraphs are descriptions of several smartphone apps that contain features we wish to emulate in our product. They solve many of the needs of our clients and if modified could be used to help more fully meet their needs. \\
				\subsection*{Whiteboard Capture Pro}
Source: Beetlebug Software's website\\
{\color{red} \url{http://www.beetlebugsoftware.com/}} \\
					
\noindent This is an iPhone app that takes a picture of a white board and then analyzes it for key content. Objects can be removed from the photograph leaving only the writing on the board behind. The App then analyzes the writing and removes the background image of the whiteboard itself. This sometimes leaves fuzz or imperfections on the white background, so there is then a slider available to filter out this extra noise/fuzz that shows up in the end product. The resulting image is a pure white background with handwriting on it.These photos can then be saved, cropped, shared, and organized with in-app tools. \\
				\subsection*{WBConference}
Source: Elecom'e website\\
{\color{red} \url{http://app.elecom.co.jp/en/wbcap/ios/manual.html}} \\
					
\noindent This is an Android app that competes with ProPANE's product because it is yet another whiteboard capturing device. WBConference differs from Whiteboard Capture Pro in that it is able to automatically recognize which sections of the image are whiteboard. This then allows it to apply its 'imagnified keystone correction' process to remove the excess background imagery. In cases that it cannot recognize the board the user can zoom in manually before capturing the image. The app has contrast adjustment and image rotation as well so that images can be captured from any angle without problems.This app has editing features as well so that postscripts or speech bubbles can be added to the images. The files can then be saved as PDFs along with any note attachments.This app has a widget for the home screen for quick image capturing, and will accept an email address for quick delivery of the images to external sources. \\
				\subsection*{CamScanner -Phone PDF Creator}
Source: Intsig's Website
{\color{red} \url{http://www.intsig.com/en/camscanner.html}} \\
					
\noindent This is the most downloaded scanner app on the market. The app takes photographs of documents, whiteboards, etc. and then runs through a user prompted editing process in which key portions of the photograph are selected, detail levels are changed, contrast, light/darkness etc are modified as well. It will then save the modified images as new documents in any number of saved folders. Notes can be attached to images they will stay with the image when it is moved. These images can be emaiedl, printed, faxed, or transferred via Bluetooth. The images can also be uploaded to google docs, evernote, skydrive, dropbox, or box.net. These documents are saved as PDFs. There are different enhancement modes: No enhance, low and high enhance, gray mode, and B\&W Document modes. These different modes will do a better job of capturing key information depending on the environment or object photographed. The B\&W mode is particularly helpful when scanning books/papers because it does a better job of removing the background noise. This app allows for batch photo taking and batch photo scanning, so that multiple pictures can be processed at the same time. \\
				\subsection*{Whiteboard Snap}
Source: Google Play Store\\
{\color{red} \url{https://play.google.com/store/apps/details?id=com.magnicode.whiteboard&hl=en}} \\
					
\noindent This app is a dumbed down version of the previous three. It does the job, it scans and enhances images, it just isnít as well known as the others and thus doesnít have the money or time to invest in extra features. This aside, it does work, it is free, and it saves images as PDFs for later use. The photos can be emailed and stored in different locations. Note attachments are available and enhancement can be used to improfe the quality of the whiteboard imagel.The greatest pro abou this application is its size. it is a much smaller program than the average whiteboard capture app.This app does a decent job as a smaller lighter free alternative. \\

     \subsection*{Conclusion}
These smart phone apps will be some of the greatest competition for ProPANE because they meet many of our client's needs already. Not only that, they're free applications and our clients wouldnt need to spend money either. The following is a list of features that ProPANE should attempt to emulate when designing its product.
...
Another key feature is the ability of these applications to save and send the images via email, google docs, and other online mediums. This will be an important point in our project as well.
...
\noindent Normally, lectures are led by the professor and do not feature collaboration, sometimes students will contribute to the board by answering questions, or in other ways.  It may be useful for the professor to track when board content is authored by a student, and the identity of that student.  ReBoard's collaboration detection could be useful in implementing a feature like this, although probably not in the initial build of the ProPANE system.  \\
  ...
  \subsection*{Camera Candidates}
Here are the current top three cameras that could help the most when building our image capturing system.
...
\noindent These cameras are interesting because they run the Android operating system. This means that custom applications could be created and run on these devices, greatly simplifying the design process. These cameras would also be useful because they could connect to Bucknell's wifi network. This would  facilitate wireless transfers of information from the cameras to whichever image processing device is chosen. The main drawbacks at the moment with these Android cameras is that two of them haven't been released yet (Samsung and Polaroid). If they are released in time, these cameras would be the most desireable of the various options available.

\subsection*{Details on Learning Disabilaties}
        \subsubsection*{Introduction}
One of the major motivating factors behind designing this image capturing system is the desire to help meet the needs of students with disabilities. The term "students with disabilities" is a very broad term, however, so the following section was created to help discribe some of the symptoms that mildly disabled students struggle with at Bucknell, and would therefore require alternative systems to capture information presented to them on whiteboards.
\\
Source: \\
...
This is just a small portion of the many disabilities faced by students in universities around the world. ProPANE hopes to help them by providing full access all information presented on boards during lectures. By providing easily accessible, easily modifiable images, ProPANE hopes to help even the playing field for students with disabilites.
Secondary goals of this project will include features that can help to make the learning process for students with disabilities even easier. Some students get distracted if they see more than one line of text at a time. If there is enough time, ProPANE will help these students by providing slide bars that will cover portions of the images that students are not currently viewing. This and many other minor features are things that ProPANE hopes to accomplish after completing its primary objectives.
