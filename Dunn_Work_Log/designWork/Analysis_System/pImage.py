...
class pImage(object):
    ...
    def cleanPass(self, shouldEnhance):
        for x in xrange(self.cellsPerRow):
            for y in xrange(self.cellsPerColumn):
                cellType = self.cells[x][y].celltype
                if cellType == pCell.STROKE:
    ...
    def make2D(self, x, y):
        ...
        make2D = make2D.point(lambda i: i > stat.mean[0] and 255)
        self.im.getColor().paste(make2D, self.cells[x][y].boundaries)
